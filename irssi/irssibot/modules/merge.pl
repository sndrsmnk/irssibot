...
if (not exists $$user_info{ircnick}) {
    public("User '$user' was not found in the database.");
    return;
}
...
return public("Nick '$irc_nick' was not found in channel '$$irc_event{target}'.") if ($ircnick_host eq "");
...
if (exists $$tmp{ircnick}) {
    public("Hostmask '$ircnick_host' for nick '$irc_nick' matches registered user '$$tmp{ircnick}'.");
    return;
}

$$state{dbh}->do("INSERT INTO ib_hostmasks (users_id, hostmask) VALUES (?, ?)", undef, $$user_info{id}, $ircnick_host);
public("Hostmask '$ircnick_host' added to '$user', '$irc_nick' is now identified.");
return;
